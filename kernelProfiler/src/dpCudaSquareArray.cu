...
int dpCudaSquareArray::execute(){
	...
	for (int i = 0; i < nKernels; i++){
		squareArray <<< nBlocks, blockSize >>> (Ain_d + (i*stride), Aout_d + (i*stride), Asize - (i*stride));
	}
	if (lastBlock != 0){
		squareArray <<<lastBlock, blockSize >>> (Ain_d + (nKernels*lastStride), Aout_d + (nKernels*lastStride), Asize - (nKernels*lastStride));
	}
